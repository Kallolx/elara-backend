...
export const scrapeKobaProducts = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    await page.goto("https://www.kobareseller.com/login", {
    ...
    if (!email || !password) {
      throw new Error(
        "Missing KOBA_EMAIL or KOBA_PASSWORD inside environment variables (.env file).",
      );
    }
    ...
    const initialTarget =
      url || "https://www.kobareseller.com/dashboard/products";
    ...
    for (let currentPage = 1; currentPage <= limit; currentPage++) {
      ...
      console.log(
        `📍 [Page ${activePage}] Fetching products from: ${currentUrlStr}...`,
      );
      ...
      const pageItems = await page.evaluate((extUrl) => {
        ...
        imgElements.forEach((img) => {
          ...
          if (
            ...
            const skuMatch =
              cardText.match(/SKU:\s*([A-Z0-9-]+)/i) ||
              cardText.match(/SKU:\s*(\d+)/i);
            const sku = skuMatch ? skuMatch[1] : "";

            const priceMatch =
              cardText.replace(/\s/g, "").match(/Price৳(\d+)/i) ||
              cardText.replace(/\s/g, "").match(/৳(\d+)/i);
            const price = priceMatch ? Number(priceMatch[1]) : 1200;

            const commissionMatch = cardText
              .replace(/\s/g, "")
              .match(/Commission৳([\d.]+)/i);
            const commission = commissionMatch ? Number(commissionMatch[1]) : 0;

            const stockMatch = cardText.match(
              /(?:Stock|Qty|Available|Quantity):\s*(\d+)/i,
            );
            let stockStatus = stockMatch ? `${stockMatch[1]} Units` : "Unknown";

            let isOutOfStock =
              /Out\s*of\s*Stock/i.test(cardText) ||
              /Sold\s*Out/i.test(cardText);
      ...
      console.log(
        `✅ Page ${activePage} done. Found ${pageItems.length} items.`,
      );

      if (pageItems.length === 0) {
        console.log(
          "🛑 No items found on this page. Terminating multi-gather early.",
        );
        break;
      }
    ...
    console.log(
      `🎉 Successfully scraped ${products.length} live reseller products!`,
    );
...
const executeIntelligentSync = async (scrapedProducts: any[]) => {
  ...
  console.log(
    "🔍 Commencing user-optimized intelligent cross-resolver pipeline...",
  );
  console.log(
    `📥 Processing ${scrapedProducts.length} scraped Koba items against ${localInventory.length} local store products.`,
  );
  ...
  for (const localProd of localInventory) {
    ...
    console.log(
      `🎯 EVALUATING PRODUCT: "${localProd.name}" (${relevantScraped.length} scraped candidates parsed)`,
    );
    ...
    if (localProd.sizes.length > 0) {
      ...
      for (const sz of localProd.sizes) {
        ...
        let bestScraped = relevantScraped.find((sc) => {
          ...
          if (cleanLabel && cleanText(sc.name).includes(cleanLabel))
            return true;
        ...
        if (bestScraped) {
          ...
          if (szSku && normalizeSku(bestScraped.sku) === szSku)
            linkageType = "🧩 SKU Match";
          else if (
            szVol &&
            parseVolume(bestScraped.name || "")?.value === szVol.value
          )
            linkageType = "⚖️ Volume Match";

          console.log(
            `    -> Sub-variant [${sz.label}] matched "${bestScraped.name}" via ${linkageType}. Supplier Out: ${isCurrentlyOut}`,
          );
          ...
          if (sz.isOutOfStock !== isCurrentlyOut) {
            ...
            hasChanged = true;
            console.log(
              `       🔥 REAL-TIME TOGGLE: Variant [${sz.label}] database value updated to: ${isCurrentlyOut ? "SOLD OUT" : "IN STOCK"}`,
            );
          } else {
            console.log(
              `       ✅ Variant [${sz.label}] database matches supplier. No update required.`,
            );
          }
        } else {
          console.log(
            `    ⚠️ Variant [${sz.label}] skipped: No precise scraped candidate located.`,
          );
        }
      }
    } else {
      ...
      console.log(
        `    -> Root Product matched "${topLevelMatchedScraped.name}". Supplier Out: ${isCurrentlyOut}`,
      );
      if (localProd.isOutOfStock !== isCurrentlyOut) {
        productStatusQueues.push({
          id: localProd.id,
        ...
        hasChanged = true;
        console.log(
          `       🔥 REAL-TIME TOGGLE: Root Product stock updated to: ${isCurrentlyOut ? "SOLD OUT" : "IN STOCK"}`,
        );
      } else {
    ...
    processedMatches.push({
      ...
      method: localSkus.includes(normalizeSku(topLevelMatchedScraped.sku))
        ? "Direct SKU"
        : "Fuzzy Name",
  ...
  for (const p of localInventory) {
    if (p.sizes.length > 0) {
      ...
      if (p.isOutOfStock !== allSizesSoldOut) {
        const existingQueueIdx = productStatusQueues.findIndex(
          (q) => q.id === p.id,
        );
  ...
  console.log(
    `⚡ Intelligent solver flushed ${operationStack.length} SQL updates!`,
  );
  return {
  ...
  try {
    ...
    if (!scrapedProducts || !Array.isArray(scrapedProducts)) {
      return res
        ...
        .json({
          success: false,
          message: "Invalid scraped products list provided.",
        });
    }
  ...
  try {
    ...
    if (activeSyncTask.isRunning) {
      return res.status(200).json({
        ...
        alreadyRunning: true,
        message:
          "An automated stock sync is already executing in the background.",
    ...
    (async () => {
      ...
      try {
        console.log(
          "🚀 Initiating Full Background Automated Stock Sync via Asynchronous Worker...",
        );
        browser = await puppeteer.launch({
          headless: true,
          args: [
        ...
        activeSyncTask.progressMsg =
          "📍 Logging into Koba reseller dashboard...";
        ...
        const password = process.env.KOBA_PASSWORD;
        if (!email || !password)
          throw new Error("Missing configuration: KOBA_EMAIL / KOBA_PASSWORD.");
        ...
        if (submitBtn) {
          ...
          await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll("button")).find(
              (b) => b.textContent?.toLowerCase().includes("log in"),
            );
            if (btn) btn.click();
          });
        }
        ...
        activeSyncTask.progressMsg =
          "🔍 Analysing local database for active SKUs to sync...";
        activeSyncTask.updatedAt = Date.now();
        ...
        console.log(
          `🎯 Targeted Sync Engine loaded ${targetSkus.length} unique SKU barcodes from Local DB.`,
        );
        if (targetSkus.length === 0) {
          throw new Error(
            "System found zero active KOBA-SKUs registered in your database!",
          );
        }
        ...
        const scrapeTargetSkuConcurrently = async (cleanSku: string) => {
          ...
          try {
            ...
            await tab.goto(
              `https://www.kobareseller.com/dashboard/products?product=${cleanSku}`,
              {
                waitUntil: "domcontentloaded",
                timeout: 20000,
              },
            ...
            const items = await tab.evaluate(() => {
              ...
              images.forEach((img) => {
                ...
                while (parent && iters < 5) {
                  if (
                    parent.textContent?.includes("SKU:") &&
            ...
            console.error(
              `   ❌ Target Search [${cleanSku}] timed out or failed:`,
              err.message,
            );
            return [];
          } finally {
        ...
        for (let i = 0; i < targetSkus.length; i += BATCH_SIZE) {
          ...
          const batchPromises = currentBatch.map((sku) =>
            scrapeTargetSkuConcurrently(sku),
          );
        ...
        await browser.close();
        console.log(
          `🏁 Targeted Precision Scan Complete! Scanned and retrieved ${allFoundProducts.length} verified products.`,
        );

        // 3. Intelligent Database Processing
        if (allFoundProducts.length === 0) {
          throw new Error(
            "Background scraper completed but no products were found on the supplier's portal.",
          );
        }
    ...
    res.status(202).json({
      success: true,
      message:
        "Intelligent Stock Sync launched successfully in a dedicated background worker thread!",
      status: "started",
    });
  } catch (error: any) {
